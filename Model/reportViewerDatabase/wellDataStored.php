...
// Retrieve reportId from POST data (ensure this is done before executing the query)
$reportId = handleUndefined($_POST['reportId']);

// Prepare and execute the query to fetch report details from the existing database
$stmtReport = $pdo1->prepare("
    SELECT r.country_id, b.region_id, s.block_id, f.field_id
    FROM tbl_report r
...
if ($reportData) {
    ...
    $fieldId = $reportData['field_id']; // Assuming field_name corresponds to fieldId

    // Check if the request method is POST
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        // Sanitize and retrieve POST data
        $eventCode = handleUndefined($_POST['eventDesc']); // Name from the form for event
        $rigName = handleUndefined($_POST['rigName']); // Name from the form for rig
        ...
        // Prepare the SQL insert statement for the existing database
        ...
        $stmt->bindParam(':eventId', $eventId); // Use the fetched event ID
        ...
        $stmt->bindParam(':rigId', $rigId); // Use the fetched rig ID
